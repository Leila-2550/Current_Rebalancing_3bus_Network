...
### 4- Inject Compensation Current at the middle of feeder 1
for (i, phase) in enumerate(["1.4", "2.4", "3.4"])
    OpenDSSDirect.Text.Command("New ISource.IDG1$i Bus1=2.$phase Phases=1  , amps = $(mag_I_comp1[i]), ang = $(ang_I_comp1[i])")
end
...
### 4- Inject Compensation Current at the middle of feeder 2
for (i, phase) in enumerate(["1.4", "2.4", "3.4"])
    OpenDSSDirect.Text.Command("New ISource.IDG2$i Bus1=4.$phase Phases=1  , amps = $(mag_I_comp2[i]), ang = $(ang_I_comp2[i])")
end
...
bar!(bus_indices .- 0.5 * bar_width, voltages_before_phase2;
    bar_width=bar_width, label="Before - Phase B", color=:green)

bar!(bus_indices .+ 0.5 * bar_width, voltages_before_phase3;
    bar_width=bar_width, label="Before - Phase C", color=:red)

bar!(bus_indices .+ 1.5 * bar_width, voltages_after_phase1;
    bar_width=bar_width, label="After - Phase A", color=:lightblue)

bar!(bus_indices .+ 2.5 * bar_width, voltages_after_phase2;
    bar_width=bar_width, label="After - Phase B", color=:lightgreen)

bar!(bus_indices .+ 3.5 * bar_width, voltages_after_phase3;
...
for el in elements
    ...
    bar(phase_indices .- bar_width/2, currents_before;
        ...
        xlabel="Phase", ylabel="Current Magnitude (A)",
        title="Current Profile for $el (Before vs. After Compensation)",
        xticks=(phase_indices, phase_labels), legend=:topright
    )

    bar!(phase_indices .+ bar_width/2, currents_after;
        bar_width=bar_width, label="After Compensation", color=:red
    )
...
bar(line_indices .- bar_width/2, line_before_mags;
    bar_width=bar_width,
    label="Before Compensation",
    color=:blue,
    xlabel="Line & Phase",
...
bar!(line_indices .+ bar_width/2, line_after_mags;
    bar_width=bar_width,
    label="After Compensation",
    color=:red
)
