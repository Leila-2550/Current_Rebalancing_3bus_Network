...
U1_before= measure_voltage("6687")
...
## Define OpenDSS circuit and model path
path = "/Users/raj055/Documents/GitHub/Current_Rebalancing_3bus_Network/data/network_N" # network with 3buses:2 load bus and 1 substation bus
...
# voltage at bus 2
_ODSS.Circuit.SetActiveBus("6925")# middle of feeder
...
volt2= _ODSS.Bus.VMagAngle()

# voltage at bus 3
_ODSS.Circuit.SetActiveBus("6694")#bus close to th eend of feeder
...
_ODSS.Circuit.SetActiveElement("Transformer.tr1")
I_sub_magang =_ODSS.CktElement.CurrentsMagAng()[9:16]# magnitude and angle of the current
[round(i, digits=3) for i in I_sub_magang]# rounding the measurement
## current unbalance factor
...
function calculate_I2()
    _ODSS.Circuit.SetActiveElement("Line.line_75588050_6732_6687") #line from substation to next bus
    currents = _ODSS.CktElement.Currents() # _ODSS.CktElement.CurrentsMagAng() # to get the magnitude and angle
    I_a = round(currents[5] , digits=2)
    I_b = round(currents[6] , digits=2)
    I_c = round(currents[7] , digits=2)
...
_ODSS.Circuit.SetActiveElement("Line.line_75588050_6732_6687")
I_l12_magang =_ODSS.CktElement.CurrentsMagAng()[9:16]
[round(i, digits=3) for i in I_l12_magang]# rounding the measurement
#####
#current I_L23
function calculate_I3()
    _ODSS.Circuit.SetActiveElement("Line.line_75588520_6925_6985")
    currents = _ODSS.CktElement.Currents() # _ODSS.CktElement.CurrentsMagAng() # to get the magnitude and angle
    I_a = round(currents[5] , digits=2)
    I_b = round(currents[6] , digits=2)
    I_c = round(currents[7] , digits=2)
...
I_lmid_before = calculate_I3()
_ODSS.Circuit.SetActiveElement("Line.line_75588520_6925_6985")
I_lmid_magang =_ODSS.CktElement.CurrentsMagAng()[9:16]
[round(i, digits=3) for i in I_lmid_magang]# rounding the measurement
#current I_Lend
function calculate_I3()
...
I_lend_before = calculate_I3()
_ODSS.Circuit.SetActiveElement("Line.line_75588076_6694_6625")
I_lend_magang =_ODSS.CktElement.CurrentsMagAng()[9:16]
[round(i, digits=3) for i in I_lend_magang]# rounding the measurement
...
######## ADD COMPENSATOR ########
## 5- Injecting the compensation current as current sources to the end of feeder
OpenDSSDirect.Text.Command("New ISource.IDG1 Bus1=6694.1.4 Phases=1  , amps = $mag1_I_comp, ang = $ang1_I_comp")
OpenDSSDirect.Text.Command("New ISource.IDG2 Bus1=6694.2.4 Phases=1  , amps = $mag2_I_comp, ang = $ang2_I_comp")
OpenDSSDirect.Text.Command("New ISource.IDG3 Bus1=6694.3.4 Phases=1  , amps = $mag3_I_comp, ang = $ang3_I_comp")
...
#get voltage and currents before compensation before solving the network with the compensation
#Retrieve all bus names in the network
all_buses = ["6687", "6925", "6694","6732","sourcebus_22000"]  # Substation, middle of feeder, end of feeder
# Function to retrieve voltage magnitudes for all buses
function get_bus_voltages()
    bus_voltages = Dict()
    for bus in all_buses
...
_ODSS.Circuit.SetActiveElement("Transformer.tr1")
I_sub_magang =_ODSS.CktElement.CurrentsMagAng()
